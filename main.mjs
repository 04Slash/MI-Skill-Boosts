export async function setup({ settings, loadModule, loadTemplates, onCharacterSelectionLoaded, onCharacterLoaded, onInterfaceReady, getResourceUrl, characterStorage }) {
	...
	const getSlot = (id) => game.equipmentSlots.getObjectByID(id),
		...
		{ addEmitters, startRenderer } = await loadModule('src/Patching.mjs'),
	...
	function generateInfotips() {
		...
		let filterInfotip = addContainer(createElement('div', { className: 'sb-pre-line', text: `${getLang('FILTER_DESC_1')}\n${getLang('FILTER_DESC_2')}\n${getLang('FILTER_DESC_3')}\n${getLang('FILTER_DESC_4')}` }));
		...
		for (let i = 6; i < 9; i++) { equipmentInfotip.append(createInfotip(getLang(`ITEM_DESC_${i}`))); };

		let consumableInfotip = addContainer(createElement('div', { className: textClass, text: getLangString('EQUIP_SLOT_Consumable') }));
		consumableInfotip.append(baseTooltip.cloneNode(true), createInfotip(getLang('ITEM_DESC_5'), null, 'warning'), createImgNode(lockedSynergyImg, 'skill-icon-xxs', getLang('LOCKED_SYNERGY')), createInfotip(getLang('ITEM_DESC_6')), createInfotip(getLang('ITEM_DESC_8')));
	...
	class SkillBoostMenu extends HTMLElement {
		constructor(skill) {
			...
			this.massFilterToggle = getElem('SB-Mass-Filtering-Toggle');
			this.massFilterLabel = getElem('SB-Mass-Filtering-Label');
			this.realmIcons = [];
			this.iconContainers = [];
			this.iconParents = [];
			...
			this.massFilterLabel.setAttribute('for', `SB-Mass-Filtering-${settingID}`);
			settingID++;
		...
		initMenu() {
			...
			if (realmOptions.length >= 2) {
				...
				this.currentRealm = realmOptions[0];
			...
			if (loadedModList.includes('Equipment Presets') && getPresetSetting('showPresetBtn'))
		...
		onSearchChange(query) {
			query = query.trim();
			let icons = skillBoosts.getRealmIcons(false, true);
			if (query === '') {
				icons.forEach(icon => {
				...
				skillBoosts.data.realmIDs.forEach(realmID => {
					if (this.currentRealm === 'Skill_Boosts:No_Realm' || this.currentRealm === realmID)
		...
		translate() {
			this.container.getElementsByTagName('sb-lang').forEach(elem => {
		...
		openPresetMenu() {
			if (!loadedModList.includes('Equipment Presets'))
				return;
	...
	class SkillBoosts {
		...
		initModifiers() {
			this.data.skills.forEach(skill => {
				this.data.realms.forEach(realm => {
					if (realm.id === 'Skill_Boosts:No_Realm')
						return;
		...
		renderEquipmentBg() {
			...
			if (getSetting('opacity'))
				this.setEquipmentOpacity();
			this.setIconSearch();
		...
		isSpecialModeActive(icon, skip = false) {
			if (filterMode && !skip) {
				this.filterIcon(icon);
				return true;
			} else if (debugEnabled && !skip) {
				console.dir(icon);
				return true;
			} else {
				return false;
		...
		getSkillIcons(skillID = this.selectedSkillID) {
			if (skillID === undefined) return [];
			return this.data.icons.get(skillID) || [];
		}
		getRealmIcons(removeRedBgs, includeFiltered = false, icons = this.getSkillIcons()) {
			let realm = this.menu !== undefined && this.menu.currentRealm;
			if (!realm) return [];
			return icons.filter(icon => icon.realms.includes(realm) && this.includeFilteredIcons(icon, includeFiltered, removeRedBgs));
		}
		getItemIcon(item, removeRedBgs, includeFiltered = false, icons = this.getSkillIcons()) {
			return icons.find(icon => icon.item === item && this.includeFilteredIcons(icon, includeFiltered, removeRedBgs));
		}
		getAllItemIcons(item) {
			let icons = [];
			this.data.skills.forEach(skill => {
				let icon = this.getItemIcon(item, false, true, this.getSkillIcons(skill.id));
				if (icon !== undefined)
					icons.push(icon);
			});
			return icons;
		}
		getCategoryIcons(category, removeRedBgs, returnIcons = false) {
			let icons = this.getSkillIcons().filter(icon => icon.category === category && this.includeFilteredIcons(icon, filterMode, removeRedBgs));
			return returnIcons ? icons : icons.map(icon => icon.item);
		}
		includeFilteredIcons(icon, includeFiltered, removeRedBgs) {
			return (!icon.isFiltered || (icon.isFiltered && includeFiltered)) && (!removeRedBgs || !(icon.hasRedBg && getSetting('hideRedBgs').includes(icon.category)));
		}
		filterIcon(oIcon) {
			let isFiltered = oIcon.isFiltered,
				icons = getSetting('massFiltering') ? this.getAllItemIcons(oIcon.item) : [oIcon];

			icons.forEach(icon => {
				let id = icon.category === 'Synergy' ? this.getSynergyID(icon.item) : icon.item.id;
				this.toggleFilterState(icon, !isFiltered, true);
				isFiltered ? this.removeValueFromMap(this.saveData, icon.skill, id) : this.addValueToMap(this.saveData, icon.skill, id);
			});
		...
		hasModifiers(skill, itemModifiers, item, isNeg = false) {
			let realms = [],
				modifiers = new Map();
		...
		filterEquipment() {
			...
			if (loadedModList.includes('[Myth] Music'))
			...
			allEquipment.forEach(item => {
				if (item.id.includes('_Lesser_Relic') && !lesserRelicDrops.some(x => x.gamemodes.includes(game.currentGamemode)))
					return;
				let isConsumable = this.isConsumable(item),
				...
				this.data.skills.forEach(skill => {
					...
					if (skill === game.thieving && item.id === 'melvorItA:Netherite_Gloves')
						return;
					let { realms, modifiers } = this.hasModifiers(skill, itemMods, item);
					if (realms.length > 0)
		...
		isConsumable(item) {
			if (item instanceof PotionItem)
				return true;
			else if (item.validSlots === undefined)
				return false;
			else if (item.validSlots.some(slot => ['melvorD:Consumable', 'melvorD:Summon1'].includes(slot.id)))
				return true;
			else
				return false;
		}
		getQty(item, isConsumable = false) {
			let quantity = game.bank.getQty(item);
			if (isConsumable)
				quantity = Math.max(Math.floor(quantity - getSetting('allbutx')), 0);
			return quantity;
		}
		SBUnequipItem(item) {
			if (!getSetting('equipFromOtherSets')) {
				notifyPlayer(-1, `[Skill Boosts]: ${getLang('SETTING_EQUIP_FROM_OTHER_SETS_ERROR')}`, 'danger', 0);
				return false;
			}
			if (player.checkIfCantEquip()) {
				return false;
			}
			let success = false;
			for (let i = 0; i < player.equipmentSets.length; i++) {
				if (i === player.selectedEquipmentSet)
					continue;

				let set = player.equipmentSets[i],
					slot = set.equipment.getSlotOfItem(item);

				if (slot) {
					const itemsToAdd = set.equipment.getItemsAddedOnUnequip(slot);
					if (!player.checkEquipRestrictionsOnUnequip(set, itemsToAdd)) {
						continue;
					}
					success = game.bank.addItem(itemsToAdd.item, itemsToAdd.quantity, false, false, false, true, `Game.Unequip`);
					if (success) {
						set.equipment.unequipItem(slot);
		...
		equipmentOnClick(icon, item, slot, ignore = false) {
			if (this.isSpecialModeActive(icon, ignore)) {
				return;
				...
				let qty = this.getQty(item, isConsumable);

				if (qty <= 0 && !isEquipped && this.SBUnequipItem(item))
					qty = this.getQty(item, isConsumable);
		...
		updateEquipmentBg(item, icon = this.getItemIcon(item), hideIcons = true) {
			if (icon === undefined)
				return;

			if (item.consumesChargesOn)
				this.updateChargeItem(item, icon);
			...
			} else if (this.getQty(item, this.isConsumable(item)) > 0)
		...
		setEquipmentOpacity() {
			let icons = [...this.getCategoryIcons('Equipment', true, true), ...this.getCategoryIcons('Consumable', true, true)].filter(x => x.realms.includes(this.menu.currentRealm) && x.item.validSlots),
			...
			game.equipmentSlots.forEach(slot => {
				...
				if (slotIcons.some(x => itemInSlot === x.item))
					occupiedSlots.set(slot, itemInSlot);
			});
			icons.forEach(icon => icon.setOpacity((icon.item.validSlots.every(x => occupiedSlots.has(x) && occupiedSlots.get(x) !== icon.item) ? 0.5 : 1)));
		}
		filterPotions() {
			game.items.potions.filter(x => this.data.skills.includes(x.action) && filterSetting(x)).forEach(potion => {
				let { realms, modifiers } = this.hasModifiers(potion.action, this.getItemMods(potion.stats), potion);
		...
		potionOnClick(potion, icon) {
			...
			if (this.isSpecialModeActive(icon)) {
				return;
			} else if (this.getQty(potion, true) > 0 && (activePotion === undefined || activePotion.item !== potion)) {
				game.potions.usePotion(potion, false);
				this.renderConsumableBg();
				this.renderConsumableQty();
			}
		}
		updateConsumableBg(consumable, icon = this.getItemIcon(consumable), hideIcons = true) {
			if (icon === undefined)
				return;

			if (player.equipment.checkForItem(consumable) || game.potions.isPotionActive(consumable) || (consumable.id === 'melvorD:Charge_Stone_of_Rhaelyx' && this.getQty(consumable, false) > 0))
				icon.setBg('sb-green-bg');
			else if (this.getQty(consumable, true) > 0)
				icon.setBg('sb-default-bg');
		...
		updateConsumableQty(consumable, icon = this.getItemIcon(consumable, true)) {
			if (icon === undefined)
				return;

			let qty = this.getQty(consumable, this.isConsumable(consumable));
		...
		setPotionOpacity() {
			let icons = this.getCategoryIcons('Consumable', true, true).filter(x => x.realms.includes(this.menu.currentRealm) && x.item instanceof PotionItem),
				skill = game.skills.getObjectByID(this.selectedSkillID);

			if (game.potions.activePotions.has(skill) && icons.some(x => game.potions.activePotions.get(skill).item === x.item))
				icons.forEach(icon => icon.setOpacity((game.potions.activePotions.get(skill).item === icon.item ? 1 : 0.5)));
			else
				icons.forEach(icon => icon.setOpacity(1));
		}
		updateChargeItem(item, icon = this.getItemIcon(item, true)) {
			if (icon === undefined)
				return;
			...
			this.data.skills.forEach(skill => {
				...
				itemIDs.forEach(itemID => {
					...
					icon.onclick = () => {
						if (this.isSpecialModeActive(icon)) {
							return;
						} else if (this.getQty(item, false) > 0) {
		...
		filterPOIs() {
			game.cartography.worldMaps.forEach(map => {
				map.pointsOfInterest.filter(x => x.activeStats.hasStats && filterSetting(x)).forEach(poi => {
					this.data.skills.forEach(skill => {
						let { realms, modifiers } = this.hasModifiers(skill, this.getItemMods(poi.activeStats));
		...
		updatePOICosts(poi, icon = this.getItemIcon(poi, true)) {
			if (icon === undefined)
				return;
		...
		updatePOIBg(poi, icon = this.getItemIcon(poi), hideIcons = true) {
			if (icon === undefined)
				return;
		...
		filterObstacles() {
			...
			let sortedObstacles = [...game.agility.sortedMasteryActions].sort((a, b) => (a.abyssalLevel - b.abyssalLevel)),
				...
				hasCombinedObstacles = this.checkForCombinedObstacles(),
				hasCombinedPillars = this.checkForCombinedPilars();
			...
			this.data.skills.forEach(skill => {
				...
				sortedObstacles.forEach(obstacle => {
					if (((hasCombinedObstacles && obstacle instanceof AgilityObstacle) || (hasCombinedPillars && obstacle instanceof AgilityPillar)) && previousObstacle && previousObstacle.category === obstacle.category)
						return;

					let obstacleMods = this.getItemMods(obstacle),
						isPositive = this.hasModifiers(skill, obstacleMods),
						realms = isPositive.realms.length > 0 ? isPositive : this.hasModifiers(skill, obstacleMods, obstacle, true);
		...
		checkForCombinedObstacles() {
			let testObstacles = game.agility.actions.getArrayFromIds(['melvorF:Waterfall', 'melvorF:LavaWaterfallDodge']);
			if (testObstacles[0].modifiers.every(x => testObstacles[1].modifiers.some(y => y.modifier === x.modifier)))
				return true;
		}
		checkForCombinedPilars() {
			let testPillars = game.agility.pillars.getArrayFromIds(['melvorF:PillarofCombat', 'melvorF:PillarofSkilling']);
			if (testPillars[0].modifiers.every(x => testPillars[1].modifiers.some(y => y.modifier === x.modifier)))
				return true;
		}
		...
		obstacleOnClick(obstacle, icon, tinyMenu = false) {
			if (this.isSpecialModeActive(icon, tinyMenu)) {
				return;
			} else {
				let buildMenu = getSetting('obstacleMenu');
		...
		setObstacleCosts(items, currencies, costsElem, tooltip) {
			...
			items.forEach(({ item, quantity }) => {
				addReq(item.media, quantity, item.name, this.getQty(item, false), item);
			});
		...
		updateObstacleBg(obstacle, icon = this.getItemIcon(obstacle), hideIcon = true) {
			if (icon === undefined)
				return;
		...
		setObstacleOpacity() {
			let icons = this.getCategoryIcons('Obstacle', true, true).filter(x => x.realms.includes(this.menu.currentRealm) && x.elem === 2);

			this.data.realms.forEach(realm => {
				if (realm === defaultRealm)
					return;

				let obstacleIcons = icons.filter(x => x.item instanceof AgilityObstacle && x.item.realm === realm);
				for (let i = 0; i < 15; i++) {
					let tierIcons = obstacleIcons.filter(x => x.item.category === i);
					if (tierIcons.some(x => x.item.isBuilt))
						tierIcons.forEach(icon => icon.setOpacity((icon.item.isBuilt ? 1 : 0.5)));
					else
						tierIcons.forEach(icon => icon.setOpacity(1));
				}

				let pillarIcons = icons.filter(x => x.item instanceof AgilityPillar && x.item.realm === realm);
				for (let i = 0; i < 2; i++) {
					let tierIcons = pillarIcons.filter(x => x.item.category === i);
					if (tierIcons.some(x => x.item.isBuilt))
						tierIcons.forEach(icon => icon.setOpacity((icon.item.isBuilt ? 1 : 0.5)));
					else
						tierIcons.forEach(icon => icon.setOpacity(1));
		...
		updateObstacleActive(obstacle, icon = this.getItemIcon(obstacle, true)) {
			if (icon === undefined)
				return;
		...
		filterPets() {
			game.pets.filter(x => !filteredPets.includes(x) && !game.petManager.unlocked.has(x) && !x.ignoreCompletion && filterSetting(x)).forEach(pet => {
				this.data.skills.forEach(skill => {
					let { realms, modifiers } = this.hasModifiers(skill, this.getItemMods(pet.stats));
					if (realms.length > 0)
						this.createIcon(pet, modifiers, realms, skill, 4, 'Pet');
				});
			});
		}
		updatePetBg(pet, icon = this.getItemIcon(pet), hideIcons = true) {
			if (icon === undefined)
				return;
		...
		filterPurchases() {
			...
			this.data.skills.forEach(skill => {
				purchases.forEach(purchase => {
					...
					let { realms, modifiers } = this.hasModifiers(skill, this.getItemMods(statObject.stats));
					if (realms.length > 0) {
		...
		updatePurchaseBg(purchase, icon = this.getItemIcon(purchase), hideIcons = true) {
			if (icon === undefined)
				return;
		...
		filterConstellations() {
			game.astrology.actions.filter(x => !game.astrology.isConstellationComplete(x) && filterSetting(x)).forEach(constellation => {
				...
				skills.forEach(skill => {
					let { realms, modifiers } = this.hasModifiers(skill, this.getConstellationModifiers(constellation));
					if (realms.length > 0)
		...
		updateDustQty() {
			dustItems.forEach(icon => icon.setText(this.getQty(icon.item, false)));
		}
		...
		updateConstellationBg(constellation, icon = this.getItemIcon(constellation), hideIcons = true) {
			if (icon === undefined)
				return;
			...
			game.summoning.synergies.forEach(synergy => {
				this.data.skills.forEach(skill => {
					if (skill === game.attack)
						return;
					let { realms, modifiers } = this.hasModifiers(skill, this.getItemMods(synergy), synergy);
					if (realms.length > 0)
		...
		synergyOnClick(synergy, icon) {
			if (this.isSpecialModeActive(icon)) {
				...
				let { equippedQty0, equippedQty1, bankQty0, bankQty1, otherSetQty0, otherSetQty1 } = this.getSynergyQty(synergy),
					currentQty0 = equippedQty0 + bankQty0,
					currentQty1 = equippedQty1 + bankQty1,
					updateQty = false;

				if (currentQty0 <= 0 && otherSetQty0 > 0 && (currentQty1 + otherSetQty1 > 0) && this.SBUnequipItem(synergy.summons[0].product)) {
					updateQty = true;
				}
				if (currentQty1 <= 0 && otherSetQty1 > 0 && (currentQty0 + otherSetQty0 > 0) && this.SBUnequipItem(synergy.summons[1].product)) {
					updateQty = true;
				}

				if (updateQty) {
					let newQty = this.getSynergyQty(synergy);
					currentQty0 = newQty.equippedQty0 + newQty.bankQty0;
					currentQty1 = newQty.equippedQty1 + newQty.bankQty1;
				}

				if (currentQty0 > 0 && currentQty1 > 0) {
					...
					slots.forEach((slot, i) => player.equipItem(synergy.summons[i].product, player.selectedEquipmentSet, slot, (i === 0 ? currentQty0 : currentQty1)));
					this.renderSynergyBg();
					this.renderSynergyQty();
				}
			}
		}
		updateSynergyBg(synergy, icon = this.getItemIcon(synergy), hideIcons = true) {
			if (icon === undefined)
				return;
		...
		updateSynergyLocked(synergy, icon = this.getItemIcon(synergy, true)) {
			...
			if (icon.synergyLocked !== undefined)
				game.summoning.isSynergyUnlocked(synergy) ? hideElement(icon.synergyLocked) : showElement(icon.synergyLocked);
		}
		getSynergyQty(synergy) {
			let quantities = {};

			synergy.summons.forEach((summon, i) => {
				quantities[`bankQty${i}`] = this.getQty(summon.product, true);
		...
		filterRelics() {
			...
			game.skills.forEach(skill => {
				...
				skill.ancientRelicSets.forEach((relicSet, realm) => {
					relicSet.relicDrops.forEach(({ relic }) => {
						if (!relicSet.foundRelics.has(relic))
							this.addValueToMap(relics, realm.id, relic);
					});
					if (relicSet.foundRelics.size !== relicSet.relicDrops.length)
						this.addValueToMap(relics, realm.id, relicSet.completedRelic);
				});
			});

			relics.forEach((relicsArr, realm) => {
				relicsArr.forEach(relic => {
					this.data.skills.forEach(skill => {
						let { realms, modifiers } = this.hasModifiers(skill, this.getItemMods(relic.stats));
						if (realms.length > 0)
							this.createIcon(relic, modifiers, [realm], skill, 4, 'Relic');
					});
				});
			});
		}
		updateRelicBg(relic, icon = this.getItemIcon(relic), hideIcons = true) {
			if (icon === undefined)
				return;
		...
		createPOISelection(container) {
			...
			pois.forEach(poi => {
				...
				this.updatePOIBg(poi, icon, false);
				this.updatePOICosts(poi, icon);
		...
		createSlotSelection(container, element, item) {
			item.validSlots.forEach(slot => {
				let existingItem = player.equipment.getItemInSlot(slot.id),
					isEmpty = existingItem === game.emptyEquipmentItem,
		...
		addCostsAndRequirements(item, modifierContainer) {
			...
			costs.items.forEach(({ item, quantity }) => {
				costContainer.append(this.createCostElement(menu, item.media, quantity, this.getQty(item, false) >= quantity));
			});
		}
		...
		getModifierNodes(statObject, negMult, posMult, includeZero = false, isConditional, isItemSynergy) {
			...
			if (statObject.conditionalModifiers !== undefined && !isConditional) {
				statObject.conditionalModifiers.forEach(conditional => {
					...
					if (desc && desc.text.includes(':') && ((conditional.modifiers && conditional.modifiers.length || 0) + (conditional.combatEffects && conditional.combatEffects.length || 0) + (conditional.enemyModifiers && conditional.enemyModifiers.length || 0)) === conditionalDescriptions.length) {
			...
			if (descriptions.length <= 0 || returnOriginal) {
		...
		createModifierTooltip(container, item, statObject, posMult = 1, negMult = 1) {
			...
			if (item.modifiedDescription && (blacklistedItems.some(x => item.id && item.id.toLowerCase().includes(x)) || item.consumesChargesOn || (modifierNodes.length <= 0 || modifierNodes[0].textContent.search('text-warning') > 0))) {
		...
		createPetTooltip(container, item) {
			...
			} else if (item.id === 'melvorF:B') {
				chanceElem.textContent += ` 1/${templateLangString('PER_TICK', { value: 1 / game.township.chanceForPet})}`;
		...
		canAccessAbyssalRealm(object) {
			return (!hasItA || (!object.abyssalLevel && (!object.slot || !object.slot.abyssalLevel)) || abyssalRealm.isUnlocked);
		}
		...
		reInitSB() {
			this.getAllIcons().forEach(icon => icon.remove());
			this.data.menus.forEach(menu => {
	...
	onCharacterSelectionLoaded(() => {
		loadedModList = mod.manager.getLoadedModList();
		// Documenation can be found at https://mod.io/g/melvoridle/m/skill-boosts //
		if (loadedModList.includes('Shamanism')) { // Replace with 'Skill Boosts' //
		...
		generalSettings.add([{
			type: 'switch',
			name: 'equipFromOtherSets',
			label: getLang('SETTING_EQUIP_FROM_OTHER_SETS'),
			hint: createElement('span', { className: 'text-danger', text: getLang('SETTING_EQUIP_FROM_OTHER_SETS_HINT') }),
			default: false
		}, {
			type: 'switch',
			...
			type: 'switch',
			name: 'opacity',
			label: getLang('SETTING_OPACITY'),
			hint: getLang('SETTING_OPACITY_HINT'),
			default: false,
			onChange: (val) => {
				if (val) {
					skillBoosts.setEquipmentOpacity();
					skillBoosts.setPotionOpacity();
					skillBoosts.setObstacleOpacity();
					skillBoosts.setSynergyOpacity();
				} else
					skillBoosts.getAllIcons().forEach(icon => icon.setOpacity(1));
			}
		}, {
			type: 'dropdown',
			name: 'obstacleMenu',
			...
			hint: createElement('div', {
				children: [
					createElement('span', { text: getLang('SETTING_ALLBUTX_DESC_1') }),
					createElement('br'),
					createElement('span', { className: 'text-danger', text: getLang('SETTING_ALLBUTX_DESC_2') }),
				]
			}),
			default: 0,
			...
			name: 'colorBgs',
			label: '',
			default: ['#005706', '#665e00', '#5c000e', '#434d5b', '#006bb8'], // #500288
			render(name, onChange, config) {
