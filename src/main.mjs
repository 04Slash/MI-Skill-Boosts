export async function setup({ settings, loadModule, loadTemplates, onModsLoaded, onInterfaceReady, getResourceUrl, characterStorage }) {
	...
	const lang = await loadModule('localization.mjs'),
		...
		getSlot = (id) => game.equipmentSlots.getObjectByID(id),
	...
	function createInfotip(text, bg, colorClass = '') {
		...
		if (bg) {
			...
			icon.classList.add('font-size-sm');
		} else if (colorClass !== '')
			icon.classList.add('badge-pill', `bg-${colorClass}`, `text-${colorClass}`, 'font-size-2sm');
		else
	...
	function generateInfotips() {
		...
		let filterInfotip = createElement('div', { className: 'sb-pre-line', text: `${getLang('FILTERING_DESC_1')}\n${getLang('FILTERING_DESC_2')}\n${getLang('FILTERING_DESC_3')}` });

		let equipmentInfotip = createElement('div', { className: 'font-w400, font-size-sm' }).appendChild(createElement('div', { className: textClass, text: getLangString('COMBAT_MISC_18') })).parentElement;
		...
		let consumableInfotip = createElement('div', { className: 'font-w400, font-size-sm' }).appendChild(createElement('div', { className: textClass, text: getLangString('EQUIP_SLOT_Consumable') })).parentElement;
		...
		let obstacleInfotip = createElement('div', { className: 'font-w400, font-size-sm' }).appendChild(createElement('div', { className: textClass, text: getLangString('GAME_GUIDE_142') })).parentElement;
		...
		let otherInfotip = createElement('div', { className: 'font-w400, font-size-sm' }).appendChild(createElement('div', { className: textClass, text: getLang('OTHERS') })).parentElement;
	...
	class SkillBoostMenu extends HTMLElement {
		...
		onSearchChange(query) {
			...
			if (query === '') {
				...
				if (['melvorD:Melvor', 'Default Sorting'].includes(this.currentRealm))
					searchKeys.push('melvorMods', 'melvorText');
				if (['melvorItA:Abyssal', 'Default Sorting'].includes(this.currentRealm))
		...
		createPresetSwal(presets, presetAPI) {
			...
			presets.forEach(preset => {
				...
				let presetIcon = textContainer.appendChild(createImgNode(presetAPI.uiGetPresetIcon(preset).media, 'resize-28 p-1'));
		...
		openPresetMenu() {
			...
			if (presets === undefined)
				return;
			let skill = skillBoosts.data.skills.find(x => x.id === skillBoosts.selectedSkill),
			...
			if (filterType === 0)
				...
				skillPresets = presets.filter(x => x.minibar.includes(skillBoosts.selectedSkill) || (skill === game.attack && x.type === 1));
	...
	class SkillBoosts {
		...
		updateForRealmChange(newRealm, icons) {
			if (!hasItA || (!this.data.realms.some(x => x.id === newRealm) && newRealm !== undefined))
				return;
		...
		moveMenu(skillID, menu = this.menu) {
			...
			skillID === 'melvorD:Attack' ? header.before(menu) : header.append(menu);
			return true;
		}
		...
		getSkillIcons(removeRedBgs, removeFiltered, removeHidden, skill = this.selectedSkill) {
			...
			let icons = this.data.icons.get(skill);
			...
			if (object.conditionalModifiers !== undefined) {
				object.conditionalModifiers.forEach(mods => {
					if (mods.modifiers !== undefined)
						mods.modifiers.forEach(mod => modifiers.push({ mod: mod, inverted: false, conditional: mods.condition }));
					if (mods.enemyModifiers !== undefined)
						mods.enemyModifiers.forEach(mod => modifiers.push({ mod: mod, inverted: true, conditional: mods.condition }));
				});
			}
			return modifiers;
		}
		hasModifiers(isNeg, skill, itemModifiers, item) {
			...
			this.data.realms.forEach(realm => {
				...
				const searchMods = this.data.modifierMap.get(realm.id, skill.id),
					...
					hasCategory = (x) => (!x.category || (x.category.realm && x.category.realm === realm) || (x.category.namespace === realm.namespace || x.category.namespace === 'melvorF' && realm === melvorRealm)),
					...
					foundMods = itemModifiers.filter(({ mod, inverted, conditional }) => isApplicable(mod, inverted) && hasRealm(mod) && hasSkill(mod) && hasCategory(mod) && hasAction(mod) && hasCurrency(mod) && hasDamageType(mod) && hasDamageType(conditional) && filterStardust(mod) && searchMods.some(y => y === mod.modifier.id));
		...
		createSlotSelection(element, item) {
			...
			let container = createElement('div');
			let textElem = container.appendChild(createElement('div', { className: 'font-w600 text-center border-bottom', text: getLang('SELECT_SLOT') }));
			let iconContainer = container.appendChild(createElement('div', { className: 'd-flex justify-content-center' }));
			item.validSlots.forEach((slot, id) => {
				...
				icon.onclick = () => { this.equipmentCallback(element, item, slot, true), this.updateSlotSelection(); };
		...
		createAgilitySelection(obstacle) {
			this.selectionObstacles = [];
			let container = createElement('div');
			let textElem = container.appendChild(createElement('div', { className: 'font-w600 text-center border-bottom', text: getLang('SELECT_OBSTACLE') }));
			let iconContainer = container.appendChild(createElement('div', { className: 'd-flex justify-content-center' }));
			let sortedObstacles = [...game.agility.sortedMasteryActions, ...game.agility.pillars.allObjects].filter(x => x.category === obstacle.category && x.realm === obstacle.realm && obstacle.slot.obstacleCount === x.slot.obstacleCount);
			sortedObstacles.forEach(obst => {
				let icon = new SkillBoostsIcon('', obst, obst.media, false, 32);
				MainTooltipController.init(icon.container);
				icon.onclick = () => { this.obstacleCallback(obst, icon), this.updateAgilitySelection(); };
				this.selectionObstacles.push({ obst, icon });
				iconContainer.append(icon);
			});
			this.updateAgilitySelection();
			return container;
		}
		updateAgilitySelection() {
			for (let i = 0; i < this.selectionObstacles.length; i++) {
				let obstacle = this.selectionObstacles[i].obst;
				let icon = this.selectionObstacles[i].icon;
				this.updateObstacleBg(obstacle, icon, false);
			}
		}
		...
		createTooltip(item) {
			...
			let container = this.createDividerElem(itemName);
			if (item instanceof EquipmentItem) {
				item.validSlots.forEach(slot => {
					container.append(createElement('small', { className: 'font-w400 font-size-sm m-1', children: [createElement('small', { className: 'sb-badge bg-primary', text: getLangString(`EQUIP_SLOT_${slot.localID}`) })] }));
				});
			}
			if ((item instanceof EquipmentItem && item.fitsInSlot("melvorD:Summon1")) || item instanceof SummoningSynergy) {
				...
				if (acquiredBy !== undefined)
					container.append(createElement('span', { className: 'text-info sb-font-2sm', text: acquiredBy }));
			}

			let modifierContainer = createElement('div', { className: 'text-success sb-font-2sm', parent: container });
			...
			if (item instanceof PointOfInterest) {
				...
				miscContainer.append(createElement('span', { className: 'text-info sb-font-2sm', text: getLangString('MENU_TEXT_PASSIVE_SLOT_COMPATIBLE') }));
