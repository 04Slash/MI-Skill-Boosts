...
function getCommonModifiers(skill) {
	...
	switch (skill.id) {
		...
		case 'melvorD:Thieving': modifiers.push('skillMasteryXPPerKo', 'autoEatThreshold', 'flatCurrencyGain', 'currencyGain', 'autoSwapFoodUnlocked', 'thievingAreaUniqueChance', 'thievingAreaUniqueChancePercent', 'thievingAutoSellPrice', 'thievingStealth', 'ignoreThievingDamageChance', 'thievingStunInterval', 'thievingStunAvoidanceChance', 'ignoreThievingDamage', 'flatThievingCurrencyGain', 'minThievingCurrencyGain', 'flatAdditionalThievingCommonDropQuantity'); break;
...
export function getMelvorModifiers(skill) {
	let combat = isCombat.some((x) => x === skill.id),
		mastery = !noMastery.some((x) => x === skill.id),
		modifiers = [...getCommonModifiers(skill), 'allowSignetDrops', 'skillXP'];
